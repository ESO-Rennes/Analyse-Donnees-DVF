...
date: "02/07/2021 - Mise à jour 04/11/2024"
output:
  html_document:
    toc: true
    toc_float:
...
# Pour rajouter des contours avec les valeurs sur les isolignes (avec spatstat)

# Convertir le SpatRaster en matrix
grille_lissee_prix_m2_matrix <- terra::as.matrix(grille_lissee_prix_m2, wide = TRUE)

# Inverser les lignes de la matrice pour corriger l'orientation
grille_lissee_prix_m2_matrix <- grille_lissee_prix_m2_matrix[nrow(grille_lissee_prix_m2_matrix):1, ]

# Récupérer les limites spatiales et la résolution
ext <- terra::ext(grille_lissee_prix_m2)
xrange <- c(ext[1], ext[2])
yrange <- c(ext[3], ext[4])

# Créer l'objet im (format spatstat) avec les limites spatiales d'origine
grille_lissee_prix_m2_im <- im(grille_lissee_prix_m2_matrix, xrange = xrange, yrange = yrange)

#paramétrage de la fenêtre graphique
opar <- par(mar = c(0,0,1.5,0))

# définition d'une palette de couleurs
palette <- colourmap(cols, breaks = bks)

# Affichage des départements de la Bretagne et des départements limitrophes en arrière-plan, en centrant la carte sur la région Bretagne
plot(st_geometry(departements), col = "grey80", border = NA, bg = "#B5D0D0", xlim = c(st_bbox(depBZH)[1], st_bbox(depBZH)[3]), ylim = c(st_bbox(depBZH)[2], st_bbox(depBZH)[4]))

# Affichage de la surface lissée 
plot(grille_lissee_prix_m2_im, col = palette, main = "", box = FALSE, ribbon = FALSE, add = TRUE)

# Affichage des isolignes avec leur valeur
contour(grille_lissee_prix_m2_im, lwd = 0.5, lty = 3, add = TRUE)
