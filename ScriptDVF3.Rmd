...
library(terra) # pour le traitement de données matricielles
...
```{r Import donnees}
DVFOK <- read.csv("DVFOK.csv", encoding="UTF-8", stringsAsFactors=FALSE)
```
...
```{r export gpkg 1, message=FALSE}
st_write(DVFgeo, "MutationsBZH.gpkg", append=FALSE)
```

## Ajout de couches géographiques externes pour les analyses/cartes

### Importer la couche des départements et la reprojeter

```{r import dpts}
departements <- st_read(dsn = "DEPARTEMENT.shp", stringsAsFactors = FALSE) # Source : Admin Express IGN (France entière)
...
```{r import communes}
Communes <- st_read(dsn = "Communes.shp", stringsAsFactors = FALSE) # source : Admin Express IGN (Bretagne)
```
...
summarise(PrixMoyen = mean(prix), Prixm2Moyen = mean(prixm2), SurfaceMoyenne = mean(surface)) # calcul long
# pour récupérer les nouvelles variables dans la couche des communes initiales
CommunesOK <- Communes2
st_geometry(CommunesOK) <- NULL
...
st_write(CommunesOK, "CommunesBZHDVF.gpkg", append=FALSE)
...
```{r calcul nb mutations et prix moyen par maille, warning=FALSE, message = FALSE, error=FALSE, echo=TRUE}
grillem2 <- grille %>% st_join(DVFgeo) %>% group_by(IDOK) %>% summarise(Nb_Mutation = n(), prixm2moyen = mean(prixm2)) # --> calcul LONG !
```
...
## Lissage spatial (échelle régionale)
...
# pour récupérer les données initiales des communes sur les centroides
CommunesBZHDVFCentroids <- SpatialPointsDataFrame(CommunesBZHDVFCentroids, CommunesBZHDVF@data)
# pour convertir le SpatialPointsDataFrame en objet geodata (format geoR requis par le package geoR pour calculer le semi-variogramme)
...
```{r Carte lissée des prix m2 agreges par commune, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r calcul dist ppv et autocorrelation spatiale 2}
# pour convertir les mutations en objet spatial (format requis par le package geoR)
MutationsBZHsp <- as(DVFgeo, "Spatial")
# Calcul sur les plus proches voisins
listPPV <- knearneigh(MutationsBZHsp@coords, k = 1) # pour connaître le plus proche voisin de chaque mutation --> calcul TRES long
...
```{r carte lissee calculee a partir mutation, echo=TRUE, message=FALSE, warning=FALSE}
# pour définir le contour de la Bretagne comme emprise pour le lissage (sinon le lissage est calculé sur une fenêtre rectangulaire)
Emprise <- as.owin(gBuffer(CommunesBZHDVF, width=0))
# pour creer un objet ppp (format spatstat) et y intégrer dedans l'emprise et les valeurs à lisser (prix moyen au m²)
...
## Cartogramme et lissage spatial (échelle régionale)
...
# pour créer un cartogramme suivant le nombre de mutations par EPCI (calcul long)
...
```{r deformation surface lissee, eval=FALSE, message=FALSE, warning=FALSE, echo=FALSE}
...
cartelissee.vecteur.drapee <- cartogramR::geom_cartogramR(cartelissee.vecteur, EPCI.BZH.cartogram)
# pour généraliser les contours de cartelissee.vecteur.drapee (on garde 5% des sommets initiaux)
cartelissee.vecteur.drapee <- rmapshaper::ms_simplify(cartelissee.vecteur.drapee, keep = 0.05, keep_shapes = TRUE)
```

### Cartogramme montrant le nombre de mutations et les prix au m² de l'immobilier lissés sur l'ensemble de la Bretagne

```{r affichage cartogramme avec surface lissee, eval=FALSE, echo=TRUE, include=FALSE}
# Paramétrage des marges de la fenêtre pour maximiser l'emprise de la carte
par(mar = c(0, 0, 1.2, 3))
# Affichage de tous les départements en arrière-plan, en centrant la carte sur la région Bretagne
plot(st_geometry(departements), col = "grey90", border = NA, bg = "#B5D0D0", xlim = c(st_bbox(Communes)[1], st_bbox(Communes)[3]), ylim = c(st_bbox(Communes)[2], st_bbox(Communes)[4])) 
...
layoutLayer(title = "Prix moyen au m² de l'immobilier et nombre de mutations par EPCI en Bretagne (2014-2019)", 
            ...
            col = "#cdd2d4", coltitle = "#8A5543", north = TRUE, tabtitle=TRUE)
...
## Elaboration d'une typologie (CAH) à partir d'indicateurs immobiliers et cartographie des sous-marchés associés (échelle régionale)
