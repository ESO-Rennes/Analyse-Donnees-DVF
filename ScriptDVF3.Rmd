...
date: "02/07/2021 - Mise à jour 24/10/2024"
output:
  html_document:
    toc: true
    toc_float:
...
library(tidyterra) # pour convertir un un objet sf volumineux très rapidement en objet SpatVector
...
# Créer un plan de calcul parallèle pour accélérer l'export
plan(multisession)
...
# Partition de l'objet sf avec les indices créés
split_sf <- split(DVFgeo, indices)

# Définir un fichier temporaire pour chaque sous-ensemble
temp_files <- paste0(tempfile(), "_", seq_along(split_sf), ".gpkg")

# Création d'une fonction pour écrire chaque sous-ensemble dans un fichier temporaire
write_subset <- function(sub_sf, file) {
  st_write(sub_sf, file, append = TRUE, quiet = TRUE)  # append pour ajouter à un même fichier
}

# Paralléliser l'écriture des sous-ensembles dans des fichiers temporaires
future_lapply(seq_along(split_sf), function(i) {
  write_subset(split_sf[[i]], temp_files[i])
})

# Assembler tous les fichiers temporaires en un seul fichier final
output_file <- "MutationsBZHt.gpkg"
for (temp_file in temp_files) {
  # Lire chaque sous-ensemble écrit
  temp_data <- st_read(temp_file, quiet = TRUE)
  
  # Écrire dans le fichier final en ajoutant les données
...
# Convertir l'objet sf en un objet spatstat de type ppp
# Extraire les coordonnées des points
coords <- st_coordinates(DVFgeo)

# Définir l'emprise où se trouvent les points
...
# récupération de l'identifiant du plus proche voisin (PPV) de chaque point
id_PPV <- nnwhich(points_ppp)
df_PPV <- as.data.frame(id_PPV)
df_PPV$id <- as.integer(rownames(df_PPV))
df_PPV <- df_PPV[ , c("id", "id_PPV")]

# Créer une liste dans laquelle chaque élément contient l'indice du PPV (format requis pour le calcul de l'indice de Moran)
PPV <- split(df_PPV$id_PPV, df_PPV$id)

# Attribuer la classe "nb" à l'objet précédent (classe requise pour le calcul de l'indice de Moran)
class(PPV) <- "nb"
...
# récupération de l'id du point de la grille le plus proche de chaque mutation

# Convertir les points de la grille sf en un objet spatstat de type ppp
# Extraire les coordonnées des points de la grille
coords_points_grille <- st_coordinates(grille)

# Définir l'emprise où se trouvent les points
emprise <- as.owin(st_bbox(grille))

# Créer un objet ppp à partir des points de la grille
points_grille_ppp <- ppp(coords_points_grille[,1], coords_points_grille[,2], window = emprise)

# Convertir les points des mutations en objet spatstat de type ppp
# Extraire les coordonnées des points des mutations
coords_mutations <- st_coordinates(DVFgeo)

# Créer un objet ppp des mutations avec le prix au m²
points_mutations_ppp <- ppp(coords_mutations[,1], coords_mutations[,2], window = emprise, marks = DVFgeo$prixm2)

# création d'un df avec pour chaque mutation l'indice du point de la grille le plus proche et la distance qui l'en sépare (fonction spatstat)
id_ppv <- nncross(points_mutations_ppp, points_grille_ppp, what = c("dist", "which"), k = 1)

#ajout de l'indice des mutations
id_ppv$id <- DVFgeo$id

# conversion des data.frames en objet data.table pour que les calculs soient rapides
id_ppv <- setDT(id_ppv)
DVFgeobis <- copy(DVFgeo) # Cette fonction crée une véritable copie indépendante de l'objet. Sans cela, DVFgeobis <- DVFgeo ne crée qu'une référence à la même structure de données en mémoire.
setDT(DVFgeobis)

# jointure pour récupérer le prix au m²
id_ppv <- merge(id_ppv, DVFgeobis[, .(id, prixm2)], by = "id", all.x = TRUE)

# pour calculer le nombre de mutations et leur prix moyen pour chaque point de la grille qui est ressorti comme plus proche voisin d'une mutation (agrégation avec la syntaxe data.table)
pts.grille.enrichie <- id_ppv[, .(Nb_Mutations = .N, 
                                 prixm2moyen = round(mean(prixm2))), 
                             by = which]

# récupération des informations précédentes dans la grille complète
grille.enrichie <- merge(grille, pts.grille.enrichie, by.x = "IDOK", by.y = "which", all.x = TRUE)

# conversion de la grille sf en objet SpatRaster
template <- rast(as_spatvector(grille.enrichie), res = cellsize) # définition des paramètres du raster (emprise, résolution, SCR)

# ajout du nombre de mutations dans la première bande
grille_spatraster <- rasterize(as_spatvector(grille.enrichie), template, field = "Nb_Mutations") 

# ajout du prix moyen au m² dans la deuxième bande
grille_spatraster <- c(grille_spatraster, rasterize(as_spatvector(grille.enrichie), template, field = "prixm2moyen"))

# nommage des bandes
names(grille_spatraster) <- c("Nb_Mutations", "prixm2moyen")

# affichage des deux bandes (optionnel)
# plot(grille_spatraster, col = (hcl.colors("Inferno", n = 5, rev = TRUE)), axes = FALSE)

# Lissage des prix moyens au m² (méthode des noyaux)

# calcul de la matrice des poids des pixels qui va décroissant en fonction du rayon et de la fonction de lissage par la méthode des noyaux (fonction terra)
...
# Habillage de la carte
mf_layout(title = "Prix moyen au m² de l'immobilier en Bretagne (2014-2019)", credits  = "", arrow = FALSE)
...
hist(terra::values(grille_lissee), nclass = 100)
